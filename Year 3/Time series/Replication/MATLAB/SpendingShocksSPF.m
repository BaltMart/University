%% This code runs VAR model with professional forecaster predictions
%% 0. PRELIMINARIES
%------------------------------------------------------------------
clear; close all; clc
warning off all
...
Xvnames = {'spfshock1','lpc_rgov','lpc_rgdp','lrwbus','tb3','amtbr','lpc_tothoursces'};
% ... and corresponding labels to be used in plots
Xvnames_long = {'SPF Shock (1Q Ahead)','Log Per Capita Real Government Spending','Log Per Capita Real GDP',...
     'Log of Real Compensation in Business','3-month Treasury bill rate',...
     'Barro-Redlick Tax Rate','Log Per Capita Real Total Hours CES'};
...
for ii=1:Xnvar
    ...
    DatesPlot(datesnum(find(ii,1)),nobs,6,'q') % Set the x-axis labels
    grid on;
end
% Save figure
print(fullfile(home_folder,'graphics','SPF_DATA.pdf'),'-dpdf');
close all
...
VARopt.FigSize = [30,70];                % size of window (figures)
...
VARopt.snames = {'\epsilon^{Gov}',...
                    ...
                    '\epsilon^{5}',...
                    '\epsilon^{6}'};
VARopt.figname = 'graphics\SPF_';% Compute impulse response
