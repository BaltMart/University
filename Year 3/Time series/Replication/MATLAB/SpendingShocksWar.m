%% This code estimates Spending shocks VAR with Ramey-Shapiro Narrative variable 
% (sudden Business Week forecast of rises in defense spending)
% Also, this is the code where the 'mistimed' wara variable is generated by
% changing variable names in the same code. Therefore, there isn't
...
delete nom denom outname deflator varname newname

%% 3. VAR ESTIMATION
%******************************************************************
% Select the list of endogenous variables...
Xvnames = {'wara','lpc_rgov','lpc_rgdp', 'lpc_rcndsv', 'lpc_rinvfx', 'lpc_tothoursces','lrwbus', 'amtbr'};
% ... and corresponding labels to be used in plots
Xvnames_long = {'Ramey-Shapiro Narrative variable','Log Per Capita Real Government Spending','Log Per Capita Real GDP',...
...
% abs(eig(VAR.F(:,2:end))) % but we can also check mannually (note, that this works for lag of one, otherwise we need to construct PHI that is in the companion form)
abs(eig(VAR.Fcomp))
disp(VARopt)
...
VARopt.snames = {'\epsilon^{Gov}',...
                    ...
                    '\epsilon^{6}',...
                    '\epsilon^{7}'};
VARopt.figname= 'graphics\WR_';% Compute impulse response
