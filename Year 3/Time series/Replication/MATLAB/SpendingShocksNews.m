%% This code runs VAR model with expected change in defence spending variable
%% 0. PRELIMINARIES
%------------------------------------------------------------------
clear; close all; clc
warning off all
...
Xvnames = {'pdvmily','lpc_rgov','lpc_rgdp','tb3','amtbr','lpc_tothours'};
% ... and corresponding labels to be used in plots
Xvnames_long = {'PDV of expected change in spending','Log Per Capita Real Government Spending','Log Per Capita Real GDP',...
     '3-month Treasury bill rate','Barro-Redlick Tax Rate','Log Per Capita Real Total Hours'};
...
% Save figure
print(fullfile(home_folder,'graphics','PDVmilly_DATA.pdf'),'-dpdf');
close all
...
VARopt.FigSize = [30,45];                % size of window (figures)
...
VARopt.snames = {'\epsilon^{Gov}',...
                    ...
                    '\epsilon^{4}',...
                    '\epsilon^{5}'};
VARopt.figname = 'graphics\PDVmillyImpulse_';% Compute impulse response
